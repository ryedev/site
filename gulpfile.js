...
// big thanks to masondesu and flackap for writing this and letting us use it!
...
// Let's crunch *local* stylesheets 
// in src/stylesheets/main.scss and put them .tmp/stylesheets
gulp.task('styles', function () {
  return gulp.src('src/stylesheets/main.scss')
    .pipe($.sass({
      ...
      sourceMap: 'sass', 
      style: 'expanded',
      // include boubon (for local stylesheets only)
...
gulp.task('asset-build', ['templates', 'styles', 'scripts'], function () {
  ...
  return gulp.src('.tmp/**/*.html')
    ...
    // useref all the html, and look in src and .tmp
...
// Look in both local src/images for images,
...
// These are just thin wrsrcers that makes the builds into deps, 
...
gulp.task('fonts', function () {
  // return gulp.src(['src/bower_components/ghost-shield/dist/webfonts/*'])
...
gulp.task('deploy', ['build-step-2'], function() {
  gulp.src("dist/**/*")
  .pipe($.ghPages({
    remoteUrl: 'git@github.com:ryedev/site.git',
    remote: 'origin',
    branch: 'gh-pages',
